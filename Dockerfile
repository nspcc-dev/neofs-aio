...
FROM ${NEOFS_HUB_IMAGE}-s3-gw:${NEOFS_S3_HUB_TAG} AS neofs-s3-gw
...
COPY ./s3-gw/wallet.json /config/wallet-s3.json
...
COPY ./s3-gw/s3.env /config/s3.env
RUN sed -ri 's,^([^=]+)=(.*)+$,\1=${\1-\2},' /config/s3.env
